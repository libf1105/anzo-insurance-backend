...
public interface ClaimMapper extends BaseMapper<Claim> {
    ...
    List<Claim> findByEnterpriseId(@Param("enterpriseId") String enterpriseId);

    /**
     * 根据状态查找理赔列表
     */
    @Select("SELECT * FROM t_claim WHERE enterprise_id = #{enterpriseId} AND status = #{status} AND deleted = false ORDER BY created_at DESC")
    List<Claim> findByStatus(@Param("enterpriseId") String enterpriseId, @Param("status") String status);
    ...
    ClaimStatistics getClaimStatistics(@Param("enterpriseId") String enterpriseId);
    ...
    List<Claim> findByReportDateRange(@Param("enterpriseId") String enterpriseId, 
